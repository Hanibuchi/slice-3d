...
def test_iter_slices_middle_is_not_empty():
    ...
    assert middle, "No slice found near the equator"
...
def test_polylines_includes_open_curves_on_nonwatertight_mesh():
    """With a non-watertight mesh, some positions have a cross-section that doesn't
    form a closed loop (it becomes an open line). Verify path.discrete drops these
    while polylines() picks them up (a regression test for a gap that actually
    occurred around y=0.15-0.22 of ammonite.glb).
    """
    ...
    assert len(section.discrete) == 0, "Known limitation: discrete doesn't pick up open cross-sections"
...
def test_slice_file_png_covers_open_curve_region(tmp_path):
    """Verify PNG output isn't empty even over a range that includes y=0.15-0.22
    (a region with open, non-closed cross-sections)."""
...
def test_slice_file_png_images_share_consistent_scale(tmp_path):
    """Verify PNGs produced by slice_file are all saved at the same pixel size
    (i.e. the same real-world scale relative to the original model) regardless of
    each cross-section's size. Previously it auto-fit to each section's content
    alone, so smaller sections got zoomed to fill the image and scales weren't
...
def test_save_section_png_without_scale_hint_still_works(tmp_path):
    """When polylines_3d/axis/bounds are not passed, verify it still saves by
    auto-fitting to the section's content as before (backward-compatibility check)."""
    mesh = make_sphere()
    s = next(s for s in slice3d.iter_slices(mesh, axis="z", thickness=2.5) if not s.is_empty)
